    ...
    void CheckVision()
{
    ...
    Vector2 origin = (Vector2)transform.position + lookDirection * 0.5f;

    Vector2 toPlayer = ((Vector2)target.transform.position - origin).normalized;
    ...
    if (angle <= visionAngle)
    {
        ...
        RaycastHit2D ray = Physics2D.Raycast(origin, toPlayer, visionRange);

        if (ray.collider != null && ray.collider.CompareTag("Player"))
        {
            hasLineOfSight = true;
            Debug.DrawRay(origin, toPlayer * visionRange, Color.green);
            return;
        }
    }

    hasLineOfSight = false;
    Debug.DrawRay(origin, lookDirection * visionRange, Color.red);
}
